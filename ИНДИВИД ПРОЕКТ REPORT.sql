...
--1)ПРОСРОЧЕННЫЙ ПАСПОРТ
SELECT TRANS_DATE AS  FRAUD_DT, PASSPORT_NUM AS PASSPORT, LAST_NAME ||' '|| FIRST_NAME ||' '|| PATRINYMIC AS FIO , PHONE , 'СОВЕРШЕНИЕ ОПЕРАЦИЙ С ПРОСРОЧЕННЫМ ПАПОРТОМ' AS FRAUD_TYPE, SYSDATE AS REPORT_DT
FROM(SELECT *
...
--2)ПРОСРОЧЕННЫЙ ДОГОВОР
SELECT TRANS_DATE AS FRAUD_DT, PASSPORT_NUM AS PASSPORT , LAST_NAME ||' '|| FIRST_NAME ||' '|| PATRINYMIC AS FIO, PHONE , 'СОВЕРШЕНИЕ ОПЕРАЦИЙ С ПРОСРОЧЕННЫМ ДОГОВОРОМ' AS FRAUD_TYPE, SYSDATE AS REPORT_DT
FROM (SELECT * 
               FROM FACT_TRANSACTIONS T
                 JOIN DIM_CARDS C
                    ON T.CARD_NUM = C.CARD_NUM
...
UNION ALL
--3) ÑÎÂÅÐØÅÍÈÅ ÎÏÅÐÀÖÈÉ Â ÐÀÇÍÛÕ ÃÎÐÎÄÀÕ Â ÒÅ×ÅÍÈÅ ÎÄÍÎÃÎ ×ÀÑÀ

select t1. td AS FRAUD_DT, c.PASSPORT_NUM AS PASSPORT,  C.LAST_NAME ||' '|| C.FIRST_NAME ||' '|| C.PATRINYMIC AS FIO, C.PHONE AS PHONE, 'ÎÏÅÐÀÖÈB Â ÐÀÇÍÛÕ ÃÎÐÎÄÀÕ Â ÒÅ×ÅÍÈÅ ÎÄÍÎÃÎ ×ÀÑÀ ' AS FRAUD_TYPE, SYSDATE AS REPORT_DT
...
UNION ALL
/* 4)Ïîïûòêà ïîäáîðà ñóìì.Â òå÷åíèå 20 ìèíóò ïðîõîäèò áîëåå 3õ îïåðàöèé ñî ñëåäóþùèì øàáëîíîì - êàæäàÿ
ïîñëåäóþùàÿ ìåíüøå ïðåäûäóùåé ïðè ýòîì îòêëîíåíû âñå êðîìå ïîñëåäíåé.Ïîñëåäíÿÿ îïåðàöèÿ (óñïåøíàÿ) â òàêîé öåïî÷êå ñ÷èòàåòñÿ ìîøåííè÷åñêîé.*/

SELECT  t6. TRANS_DATE AS FRAUD_DT, c.PASSPORT_NUM AS PASSPORT,  C.LAST_NAME ||' '|| C.FIRST_NAME ||' '|| C.PATRINYMIC AS FIO, C.PHONE AS PHONE, 'ÏÎÏÛÒÊÀ ÏÎÄÁÎÐÀ ÑÓÌÌ ' AS FRAUD_TYPE, SYSDATE  AS REPORT_DT           
FROM(SELECT T5.*
FROM(SELECT T4.* , SUM(CC) OVER (PARTITION BY RAN, CARD_NUM )   AS SU2       
FROM(SELECT T3.*,  count(AMT)  OVER (PARTITION BY  CARD_NUM,  RAN ORDER BY TRANS_DATE ROWS  BETWEEN UNBOUNDED PRECEDING AND 1 PRECEDING  ) AS CO, -- ÁÎËÅÅ ÒÐÅÕ ÎÏÅÐÀÖÈÉ 
                                  LEAD(amt) OVER (PARTITION BY card_num ORDER BY trans_date) AS LS,  
                                  CASE WHEN  Amt>COALESCE(LEAD(amt) OVER (PARTITION BY card_num ORDER BY trans_date),0) THEN 0 ELSE 1 END AS CC -- ÂÛßÂËßÅÌ ÎÏÅÐÀÖÈÈ ÍÅ ÑÎÎÒÂÅÒÑÒÂÓÞÙÈÅ ÓÑËÎÂÈÞ ÊÀÆÄÀß ÌÅÍÜØÅ ÏÐÅÄÈÄÓÙÅÉ
            FROM(SELECT T2.*, DENSE_RANK() OVER (PARTITION BY CARD_NUM  ORDER BY TIM) AS RAN  --ÐÀÍÆÈÐÓÅÌ ÎÏÅÐÀÖÈÈ ,ÊÎÒÎÐÛÅ ÂÕÎÄßÒ Â ÈÍÒÅÐÂÀË È ÍÅ ÂÕÎÄßÒ
                          FROM (select t.*,count(AMT)  OVER (PARTITION BY  CARD_NUM  ORDER BY TRANS_DATE ROWS  BETWEEN UNBOUNDED PRECEDING AND 1 PRECEDING  ) AS SU,
                                                   ...
                                                   THEN 1 ELSE 0 END AS TIM --ÂÕÎÄÈÒ ËÈ ÎÏÅÐÀÖÈß Â ÈÍÒÅÐÂÀË 20 ÌÈÍ
