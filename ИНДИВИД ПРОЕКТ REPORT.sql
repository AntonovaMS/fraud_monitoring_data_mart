...
select t1. td AS FRAUD_DT, c.PASSPORT_NUM AS PASSPORT,  C.LAST_NAME ||' '|| C.FIRST_NAME ||' '|| C.PATRINYMIC AS FIO, C.PHONE AS PHONE, 'ОПЕРАЦИИ В РАЗНЫХ ГОРОДАХ В ТЕЧЕНИЕ ОДНОГО ЧАСА' AS FRAUD_TYPE, SYSDATE AS REPORT_DT
...
UNION ALL
/* 4)Попытка подбора сумм.В течение 20 минут проходит более 3х операций со следующим шаблоном - каждая
последующая меньше предыдущей при этом отклонены все кроме последней.Последняя операция (успешная) в такой цепочке считается мошеннической.*/


SELECT  t6. TRANS_DATE AS FRAUD_DT, c.PASSPORT_NUM AS PASSPORT,  C.LAST_NAME ||' '|| C.FIRST_NAME ||' '|| C.PATRINYMIC AS FIO, C.PHONE AS PHONE, 'ПОПЫТКА ПОДБОРА СУММ' AS FRAUD_TYPE, SYSDATE  AS REPORT_DT           
FROM(SELECT T5.*
FROM(SELECT T4.* , SUM(CC) OVER (PARTITION BY RAN, CARD_NUM )   AS SU2       
FROM(SELECT T3.*,  count(AMT)  OVER (PARTITION BY  CARD_NUM,  RAN ORDER BY TRANS_DATE ROWS  BETWEEN UNBOUNDED PRECEDING AND 1 PRECEDING  ) AS CO, -- БОЛЕЕ ТРЕХ ОПЕРАЦИЙ  
                                  LEAD(amt) OVER (PARTITION BY card_num ORDER BY trans_date) AS LS,  
                                  CASE WHEN  Amt>COALESCE(LEAD(amt) OVER (PARTITION BY card_num ORDER BY trans_date),0) THEN 0 ELSE 1 END AS CC -- ВЫЯВЛЯЕМ ОПЕРАЦИИ НЕ СООТВЕТСТВУЮЩИЕ УСЛОВИЮ КАЖДАЯ МЕНЬШЕ ПРЕДИДУЩЕЙ
            FROM(SELECT T2.*, DENSE_RANK() OVER (PARTITION BY CARD_NUM  ORDER BY TIM) AS RAN  --РАНЖИРУЕМ ОПЕРАЦИИ ,КОТОРЫЕ ВХОДЯТ В ИНТЕРВАЛ И НЕ ВХОДЯТ
                          ...
                          FROM (select t.*,count(AMT)  OVER (PARTITION BY  CARD_NUM  ORDER BY TRANS_DATE ROWS  BETWEEN UNBOUNDED PRECEDING AND 1 PRECEDING  ) AS SU,
                                                   ...
                                                   THEN 1 ELSE 0 END AS TIM --ВХОДИТ ЛИ ОПЕРАЦИЯ В ИНТЕРВАЛ 20 МИН
